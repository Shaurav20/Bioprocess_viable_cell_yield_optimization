# Bioprocess_viable_cell_yield_optimization
Bioprocess viable cell yield optimization model building with jmp
...
Changed Experiment column to nominal type for categorical analysis
...
3. Statistical Testing & Decision Making
...
4. Polynomial Degree Determination
...
7. Optimization via Prediction Profiler
...
Glucose: [Optimal value]

Glutamine: [Optimal value]

Time: [Optimal value]

Temperature: [Optimal value]

pH: [Optimal value]

Feed volume: [Optimal value]
...
Visual Verification: Fit Y by X plot (Actual vs Predicted) showed points clustered around diagonal
...
Saved the screenshots (at maximum desirability setting) as ss1 and ss2 png files

Key Insights

Time relationship is cubic, representing sigmoidal cell growth patterns

Experiment effects are random, not fixed, requiring mixed modeling
...
JMP Scripting Language (JSL) compatibility

Basic statistical knowledge for interpretation

How to Reproduce

Load a Kaggle bioprocess-based dataset into JMP. The dataset used in building this model is present here as Bioprocess_data.csv file. Also the Bioprocess_data.jmp, Training_data.jmp, Validation_data.jmp, Testing_data.jmp and Simulation_data.jmp files are also present here

Preprocess the data according to the previously stated instructions

Execute Model_3.jsl

Use Model_simulation.jsl to reproduce optimization

Refer to reports for interpretation guidance
...
Convert model to deployable format for production use
